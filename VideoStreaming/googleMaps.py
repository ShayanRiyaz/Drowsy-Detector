...
def get_time_and_distance(route_json, distance=True):
    if not distance:
        ...
        for i in range(len(route_json['routes'][0]['legs'][0]['steps'])):
            distance_and_time[i] = (route_json['routes'][0]['legs'][0]['steps'][i]['distance']['value'],
                                    route_json['routes'][0]['legs'][0]['steps'][i]['duration']['value'])
...
def get_lat_long_from_route(route_json):
    ...
    for i in range(len(route_json['routes'][0]['legs'][0]['steps'])):
        journey_coords[i] = (route_json['routes'][0]['legs'][0]['steps'][i]['end_location']['lat'],
                             route_json['routes'][0]['legs'][0]['steps'][i]['end_location']['lng'])
